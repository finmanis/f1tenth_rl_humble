...
Usage:
    ...
    # Larger model for longer/multi-map training
    python scripts/train.py --config configs/custom.yaml --size medium
    python scripts/train.py --config configs/custom.yaml --size large --maps ... --steps-per-map 2000000
"""
...
def parse_args():
    ...
    parser.add_argument("--opponent-line", type=str, default=None,
                        choices=["random", "center"],
                        help="Opponent line selection: random (left/center/right each episode) "
                             "or center (centerline only)")
...
def apply_overrides(config, args):
    ...
    # Opponent line mode
    if args.opponent_line:
        if "multi_agent" not in config:
            config["multi_agent"] = {}
        config["multi_agent"]["opponent_random_lines"] = (args.opponent_line == "random")
